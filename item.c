// build:
// emcc -O3 item.c -o item.js -s WASM=1 -s EXPORT_NAME="'ItemGeneratorModule'" -s MODULARIZE=1
// import:
...
ig_int ig_prng(ig_int min, ig_int max) {
	if (max - min <= 1) {
		return min;
	}
...
void ig_decrement(struct effect* summary, ig_int length, struct effect* effect) {
	for (int32_t i = 0; i < length; ++i) {
...
ig_int ig_pick(struct group* group, ig_int length, struct group** res, struct group** parent) {
...
ig_int ig_select(ig_int count, struct group* group, ig_int length, struct group** res, struct group** parents) {
  ig_int err = 0;
...
ig_int ig_simulate(ig_int* counts, struct group** groups, ig_int* lengths, ig_int length1, struct group** selected, struct group** selectedParents) {
  ig_int err = 0;
...
EMSCRIPTEN_KEEPALIVE ig_int ig_generate(ig_int amount, ig_int* counts, struct group** groups, ig_int* lengths, ig_int length1, struct effect* summary, ig_int length2, ig_int* seed, struct effect* extras, ig_int length4) {
  ...
  if (!extras && length4 > 0) {
	...
	struct group** selected = malloc(sizeof(struct group*) * length3);
	struct group** selectedParents = malloc(sizeof(struct group*) * length3);
	struct effect* scratch = malloc(sizeof(struct effect) * length2);
...
EMSCRIPTEN_KEEPALIVE struct group** ig_prepare_group_list(ig_int length) {
  struct group** groups = ig_alloc(sizeof(struct group*) * length);
  return groups;
}

EMSCRIPTEN_KEEPALIVE struct group* ig_prepare_group(ig_int length) {
  struct group* group = ig_alloc(sizeof(struct group) * length);
  return group;
}

EMSCRIPTEN_KEEPALIVE struct effect* ig_prepare_effects(ig_int length) {
  struct effect* effects = ig_alloc(sizeof(struct effect) * length);
  return effects;
}

EMSCRIPTEN_KEEPALIVE ig_int* ig_prepare_ints(ig_int length) {
  ig_int* ints = ig_alloc(sizeof(ig_int) * length);
  return ints;
}
